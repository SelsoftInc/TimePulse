...
const TimesheetApproval = () => {
  ...
  const handleApproval = async (timesheetId, action, comments = '') => {
    ...
    try {
      ...
      const emailNotification = {
        ...
        subject: `Timesheet ${action === 'approve' ? 'Approved' : 'Rejected'} - Week ${timesheet.weekRange}`,
        body: `
          Dear ${timesheet.employeeName},
          
          Your timesheet for the week ${timesheet.weekRange} has been ${action === 'approve' ? 'approved' : 'rejected'}.
      ...
      setTimesheets(prevTimesheets => 
        prevTimesheets.map(ts => 
          ts.id === timesheetId 
            ? { ...ts, status: action === 'approve' ? 'Approved' : 'Rejected', approvalComments: comments }
            : ts
      ...
      successMessage.className = `toast-notification ${action === 'approve' ? 'success' : 'warning'}`;
      successMessage.innerHTML = `
        <div class="toast-icon">
          <i class="fa fa-${action === 'approve' ? 'check-circle' : 'exclamation-circle'}"></i>
        </div>
        ...
        <div className="approval-modal">
          <div className="approval-modal-content">
            <div className="approval-modal-header">
              <h5 className="approval-modal-title">Review Timesheet - {timesheet.employeeName}</h5>
              <button type="button" className="approval-modal-close" onClick={onClose}>
              ...
              <div className="approval-info-grid">
                <div className="approval-info-section employee-section">
                  <div className="employee-avatar">
                    {timesheet.employeeName.split(' ').map(name => name[0]).join('')}
                  </div>
                ...
                <div className="approval-info-section timesheet-details">
                  ...
                  <div className="detail-grid">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{timesheet.weekRange}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Client:</span>
                      <span className="detail-value">
                        {timesheet.clientName}
                        <span className="client-type-badge">{timesheet.clientType}</span>
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Billable Hours:</span>
                      <span className="detail-value hours-value">{timesheet.billableProjectHrs}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Time Off/Holiday:</span>
                      <span className="detail-value">{timesheet.timeOffHolidayHrs}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Total Hours:</span>
                      <span className="detail-value total-hours">{timesheet.totalTimeHours}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Submitted:</span>
                      <span className="detail-value">{timesheet.submittedDate}</span>
                    </div>
                  </div>
                </div>
                    ...
                    {timesheet.attachments.length > 0 ? (
                      timesheet.attachments.map((attachment, index) => (
                        <div key={index} className="attachment-item">
                          <i className={`fa ${attachment.endsWith('.pdf') ? 'fa-file-pdf' : 
                                        attachment.endsWith('.docx') || attachment.endsWith('.doc') ? 'fa-file-word' : 
                                        'fa-file'}`}></i>
              ...
              <div className="approval-action-section">
                ...
                <form onSubmit={handleSubmit}>
                  <div className="approval-options">
                    <div className={`approval-option ${action === 'approve' ? 'selected' : ''}`} onClick={() => setAction('approve')}>
                      <input 
                        type="radio" 
                        name="action" 
                        id="approve"
                      ...
                      <label htmlFor="approve">
                        ...
                        <div className="option-text">
                          ...
                          <span className="option-desc">Timesheet meets requirements</span>
                        </div>
                      </label>
                    </div>
                    
                    <div className={`approval-option ${action === 'reject' ? 'selected' : ''}`} onClick={() => setAction('reject')}>
                      <input 
                        type="radio" 
                        name="action" 
                        id="reject"
                      ...
                      <label htmlFor="reject">
                        ...
                        <div className="option-text">
                          ...
                          <span className="option-desc">Requires corrections</span>
                        </div>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="comments-section">
                    <label>
                      Comments {action === 'reject' && <span className="required-mark">*</span>}
                    </label>
                    <textarea
                      rows="3"
                      value={comments}
                  ...
                  <div className="approval-actions">
                    ...
                    <button 
                      type="submit" 
                      className={`submit-button ${action === 'approve' ? 'approve' : 'reject'}`}
                    >
                      <i className={`fa ${action === 'approve' ? 'fa-check' : 'fa-times'}`}></i>
  ...
  const getStatusBadge = (status) => {
    ...
    const config = statusConfig[status] || { class: 'status-unknown', icon: 'fa-question-circle' };
      ...
      <div className="approval-container">
        <div className="approval-header">
          <div className="header-content">
            ...
            <p className="page-subtitle">Review and manage employee timesheet submissions</p>
          </div>
        ...
        <div className="approval-stats">
          <div className="stat-card pending">
            ...
            <div className="stat-content">
              <h3>{filteredTimesheets.filter(t => t.status === 'Submitted for Approval').length}</h3>
              <p>Pending</p>
            </div>
          </div>
                  ...
                  <div key={timesheet.id} className="timesheet-card">
                    ...
                    <div className="card-body">
                      ...
                      <div className="timesheet-detail">
                        ...
                        <span>
                          {timesheet.clientName}
                          <span className="client-type">{timesheet.clientType}</span>
                        </span>
                      </div>
                      ...
                      <div className="timesheet-hours">
                        ...
                        <div className="hours-item">
                          ...
                          <span className="hours-value total">{timesheet.totalTimeHours}</span>
                        </div>
                      </div>
                      ...
                      {timesheet.attachments.length > 0 && (
                        <div className="attachment-indicator">
                          ...
                          <span>{timesheet.attachments.length} attachment{timesheet.attachments.length !== 1 ? 's' : ''}</span>
                        </div>
                      )}
                    </div>
