...
export const validateWeekdays = (startDate, endDate) => {
  ...
  if (isWeekend(start) || isWeekend(end)) {
    return { 
      isValid: false, 
      message: "Dates cannot be weekends (Saturday/Sunday)" 
    };
  }
  ...
  while (currentDate <= end) {
    if (isWeekend(currentDate)) {
      return { 
        isValid: false, 
        message: "Date range cannot include weekends (Saturday/Sunday)" 
      };
    }
...
export const formatTaxId = (taxId) => {
  const cleanTaxId = taxId.replace(/\D/g, '');
  if (cleanTaxId.length === 9) {
    return `${cleanTaxId.slice(0, 3)}-${cleanTaxId.slice(3, 5)}-${cleanTaxId.slice(5)}`;
  }
  return taxId;
};
